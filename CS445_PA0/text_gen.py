...
def calculate_bigrams(word_list):
    ...
    bigrams[None] = [word_list[0]]

    # Create bigrams dict
    for i in range (0, len(word_list) - 1):
        if word_list[i] in bigrams:
            bigrams[word_list[i]].append(word_list[i+1])
        else:
            bigrams[word_list[i]] = [word_list[i+1]]

    # Calculate unigrams in bigrams
    for key, value in bigrams.items():
        bigrams[key] = calculate_unigrams(value)
...
def calculate_trigrams(word_list):
    ...
    trigrams[(None, None)] = [word_list[0]]
    trigrams[(None, word_list[0])] = [word_list[1]]

    # Create trigrams dict
    for i in range (0, len(word_list) - 2):
        if (word_list[i], word_list[i+1]) in trigrams:
            trigrams[(word_list[i], word_list[i+1])].append(word_list[i+2])
        else:
            trigrams[(word_list[i], word_list[i+1])] = [word_list[i+2]]

    # Calculate unigrams in trigrams
    for key, value in trigrams.items():
        trigrams[key] = calculate_unigrams(value)
...
def random_trigram_text(first_word, second_word, bigrams, trigrams, num_words):
    ...
    for i in range(num_words - 2):
        if (first_word, second_word) in trigrams:
            ...
            next_word = select_random(bigrams[second_word])
